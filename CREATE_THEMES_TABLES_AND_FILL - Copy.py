...
for row in vuka:
    ...
    c2.execute('''create table "['''+tableId+'''] '''+tableName+'''" ("Sales_period" TEXT UNIQUE, "Sales" DEFAULT (null))''')


    #####################################################

    # FILL UP THEME WITH AVAILABLE TIME ROWS
    c2.execute('''INSERT INTO "['''+tableId+'''] '''+tableName+'''" SELECT "Sales_period", "Sales" FROM "'''+tbl_Topweek_Log+'''" WHERE "Item_id"="'''+tableId+'''"''')

    #####################################################

    # FILL UP THEME TABLE WITH UN-AVAILABLE TIME ROWS
    c2.execute('''INSERT OR IGNORE INTO "['''+tableId+'''] '''+tableName+'''" SELECT "Sales_period", "Sales" FROM "'''+tbl_Fill_Daterange+'''"''')
